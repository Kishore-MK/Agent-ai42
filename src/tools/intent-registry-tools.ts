...
import { idl } from "../lib/idl.js"; // Your agent registry IDL
...
export function generateIntentHash(): number[] {
...
// Register Agent
export async function registerAgent(agentId: number[]) {
  const [registryPDA] = PublicKey.findProgramAddressSync(
    [Buffer.from("registry")],
    program.programId
  );
...
export async function updateAgentStatus(agentPDA: PublicKey, isActive: boolean) {
  const [registryPDA] = PublicKey.findProgramAddressSync(
    [Buffer.from("registry")],
    program.programId
  );
...
// 5. Verify Intent
export async function verifyIntent(intentPDA: PublicKey) {
...
export async function executeIntent(actionId:string, intentPDA: PublicKey, agentPDA: PublicKey, amount:number,agentSecretKey:string,agentPublicKey:PublicKey) { 
    const timestamp = Math.floor(Date.now() / 1000);
  
    const message = `${actionId}${amount}${timestamp}`;
    const messageBytes = new TextEncoder().encode(message);
    const secretKey = bs58.decode(agentSecretKey);
    ...
    const merchantProof = {
      ...
      merchantPublicKey: agentPublicKey.toBytes(),
    };
...
// 7. Revoke Intent
export async function revokeIntent(intentPDA: PublicKey) {
  const tx = await program.methods
    .revokeIntent()
    .accounts({
      intent: intentPDA,
...
// 9. Get Agent Score
export async function getAgentScore(agentPDA: PublicKey) {
